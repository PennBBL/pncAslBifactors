...
######################################################
#### FIGURE X: CORR TRAITS BY SCREENING CATEGORY ####
######################################################
...
for (i in 1:13){
        ...
        facTbl[i,1]<-mean(subjData$MOOD_CorrTraits[which(y==1)],na.rm=TRUE)
        facTbl[i,2]<-sd(subjData$MOOD_CorrTraits[which(y==1)],na.rm=TRUE)/sqrt(length(which(y==1)))

        facTbl[i,3]<-mean(subjData$PSYCHOSIS_CorrTraits[which(y==1)],na.rm=TRUE)
        facTbl[i,4]<-sd(subjData$PSYCHOSIS_CorrTraits[which(y==1)],na.rm=TRUE)/sqrt(length(which(y==1)))

        facTbl[i,5]<-mean(subjData$EXTERNALIZING_CorrTraits[which(y==1)],na.rm=TRUE)
        facTbl[i,6]<-sd(subjData$EXTERNALIZING_CorrTraits[which(y==1)],na.rm=TRUE)/sqrt(length(which(y==1)))

        facTbl[i,7]<-mean(subjData$FEAR_CorrTraits[which(y==1)],na.rm=TRUE)
        facTbl[i,8]<-sd(subjData$FEAR_CorrTraits[which(y==1)],na.rm=TRUE)/sqrt(length(which(y==1)))
...
ggsave(file="/data/joy/BBL/projects/pncAslAcrossDisorder/TablesFigures/Figure_CorrTraitsByDiag.png", width = 15, height = 5, units = "in", dpi = 300)
...
#First, define lists ####HARDCODED COLUMN NUMBERS####
Asl_Region_List <- names(subjData)[2900:2907]
...
pvalues <- as.data.frame(rbind(Add_RegionModels_p, Agr_RegionModels_p, Con_RegionModels_p, Gad_RegionModels_p, Mdd_RegionModels_p, Ocd_RegionModels_p, Odd_RegionModels_p, Ps_RegionModels_p, Ptd_RegionModels_p, Sep_RegionModels_p, Soc_RegionModels_p, Sph_RegionModels_p))

#Add column names
colnames(pvalues) <- c("OverallPsy_RightPostcentralGyrus","OverallPsy_MidbrainSubgenACC","OverallPsy_LeftParahippocampalMidbrain","OverallPsy_LeftRostralACC","OverallPsy_RightDorsalACC","Psychosis_LeftFrontalOperculumInsula","Fear_RightOccipFusiform","Fear_LeftSubgenACC")

#FDR correct p-values across diagnoses for each cluster separately
OverallPsy_RightPostcentralGyrus_fdr <- p.adjust(pvalues$OverallPsy_RightPostcentralGyrus,method="fdr")
OverallPsy_MidbrainSubgenACC_fdr <- p.adjust(pvalues$OverallPsy_MidbrainSubgenACC,method="fdr")
OverallPsy_LeftParahippocampalMidbrain_fdr <- p.adjust(pvalues$OverallPsy_LeftParahippocampalMidbrain,method="fdr")
OverallPsy_LeftRostralACC_fdr <- p.adjust(pvalues$OverallPsy_OverallPsy_LeftRostralACC,method="fdr")
OverallPsy_RightDorsalACC_fdr <- p.adjust(pvalues$OverallPsy_RightDorsalACC,method="fdr")
Psychosis_LeftFrontalOperculumInsula_fdr <- p.adjust(pvalues$Psychosis_LeftFrontalOperculumInsula,method="fdr")
Fear_RightOccipFusiform_fdr <- p.adjust(pvalues$Fear_RightOccipFusiform,method="fdr")
Fear_LeftSubgenACC_fdr <- p.adjust(pvalues$Fear_LeftSubgenACC,method="fdr")
...
###NOTE: the coefficient is ####HARDCODED##### at t$coefficients[6,1], if you change the number of variables in your model, this number will also have to be changed.
dCov<-function(x){
  t<-summary(x)
...
#test<-lm(LM_asl_OverallPsy_RightPostcentralGyrus~ age + sex + I(scale(age, scale=FALSE, center=TRUE)^2) + age*sex + I(scale(age, scale=FALSE, center=TRUE)^2)*sex + pcaslRelMeanRMSMotion + Add, data = AddData)
...
#############################################################
#### FIGURE X: REGION EFFECT SIZES BY SCREENING CATEGORY ####
#############################################################

##Create table with region effect sizes only for diagnoses with >20 subjects.
facTbl3<-as.data.frame(matrix(nrow=12,ncol=8))
colnames(facTbl3)[1]<-"OverallPsy_RightPostcentralGyrus"
colnames(facTbl3)[2]<-"OverallPsy_MidbrainSubgenACC"
colnames(facTbl3)[3]<-"OverallPsy_LeftParahippocampalMidbrain"
colnames(facTbl3)[4]<-"OverallPsy_LeftRostralACC"
colnames(facTbl3)[5]<-"OverallPsy_RightDorsalACC"
colnames(facTbl3)[6]<-"Psychosis_LeftFrontalOperculumInsula"
colnames(facTbl3)[7]<-"Fear_RightOccipFusiform"
colnames(facTbl3)[8]<-"Fear_LeftSubgenACC"
...
##Add effect sizes to table
	 ...
	 facTbl3[1,4]<-Add_RegionModels_asl_d[4]
         facTbl3[1,5]<-Add_RegionModels_asl_d[5]
         facTbl3[1,6]<-Add_RegionModels_asl_d[6]
	 facTbl3[1,7]<-Add_RegionModels_asl_d[7]
         facTbl3[1,8]<-Add_RegionModels_asl_d[8]

         facTbl3[2,1]<-Agr_RegionModels_asl_d[1]
         facTbl3[2,2]<-Agr_RegionModels_asl_d[2]
         facTbl3[2,3]<-Agr_RegionModels_asl_d[3]
	 facTbl3[2,4]<-Agr_RegionModels_asl_d[4]
         facTbl3[2,5]<-Agr_RegionModels_asl_d[5]
         facTbl3[2,6]<-Agr_RegionModels_asl_d[6]
	 facTbl3[2,7]<-Agr_RegionModels_asl_d[7]
         facTbl3[2,8]<-Agr_RegionModels_asl_d[8]

         facTbl3[3,1]<-Con_RegionModels_asl_d[1]
         facTbl3[3,2]<-Con_RegionModels_asl_d[2]
         facTbl3[3,3]<-Con_RegionModels_asl_d[3]
	 facTbl3[3,4]<-Con_RegionModels_asl_d[4]
         facTbl3[3,5]<-Con_RegionModels_asl_d[5]
         facTbl3[3,6]<-Con_RegionModels_asl_d[6]
	 facTbl3[3,7]<-Con_RegionModels_asl_d[7]
         facTbl3[3,8]<-Con_RegionModels_asl_d[8]

         facTbl3[4,1]<-Gad_RegionModels_asl_d[1]
         facTbl3[4,2]<-Gad_RegionModels_asl_d[2]
         facTbl3[4,3]<-Gad_RegionModels_asl_d[3]
	 facTbl3[4,4]<-Gad_RegionModels_asl_d[4]
         facTbl3[4,5]<-Gad_RegionModels_asl_d[5]
         facTbl3[4,6]<-Gad_RegionModels_asl_d[6]
	 facTbl3[4,7]<-Gad_RegionModels_asl_d[7]
         facTbl3[4,8]<-Gad_RegionModels_asl_d[8]

         facTbl3[5,1]<-Mdd_RegionModels_asl_d[1]
         facTbl3[5,2]<-Mdd_RegionModels_asl_d[2]
         facTbl3[5,3]<-Mdd_RegionModels_asl_d[3]
	 facTbl3[5,4]<-Mdd_RegionModels_asl_d[4]
         facTbl3[5,5]<-Mdd_RegionModels_asl_d[5]
         facTbl3[5,6]<-Mdd_RegionModels_asl_d[6]
	 facTbl3[5,7]<-Mdd_RegionModels_asl_d[7]
         facTbl3[5,8]<-Mdd_RegionModels_asl_d[8]

         facTbl3[6,1]<-Ocd_RegionModels_asl_d[1]
         facTbl3[6,2]<-Ocd_RegionModels_asl_d[2]
         facTbl3[6,3]<-Ocd_RegionModels_asl_d[3]
	 facTbl3[6,4]<-Ocd_RegionModels_asl_d[4]
         facTbl3[6,5]<-Ocd_RegionModels_asl_d[5]
         facTbl3[6,6]<-Ocd_RegionModels_asl_d[6]
	 facTbl3[6,7]<-Ocd_RegionModels_asl_d[7]
         facTbl3[6,8]<-Ocd_RegionModels_asl_d[8]

         facTbl3[7,1]<-Odd_RegionModels_asl_d[1]
         facTbl3[7,2]<-Odd_RegionModels_asl_d[2]
         facTbl3[7,3]<-Odd_RegionModels_asl_d[3]
	 facTbl3[7,4]<-Odd_RegionModels_asl_d[4]
         facTbl3[7,5]<-Odd_RegionModels_asl_d[5]
         facTbl3[7,6]<-Odd_RegionModels_asl_d[6]
	 facTbl3[7,7]<-Odd_RegionModels_asl_d[7]
         facTbl3[7,8]<-Odd_RegionModels_asl_d[8]

         facTbl3[8,1]<-Ps_RegionModels_asl_d[1]
         facTbl3[8,2]<-Ps_RegionModels_asl_d[2]
         facTbl3[8,3]<-Ps_RegionModels_asl_d[3]
	 facTbl3[8,4]<-Ps_RegionModels_asl_d[4]
         facTbl3[8,5]<-Ps_RegionModels_asl_d[5]
         facTbl3[8,6]<-Ps_RegionModels_asl_d[6]
	 facTbl3[8,7]<-Ps_RegionModels_asl_d[7]
         facTbl3[8,8]<-Ps_RegionModels_asl_d[8]

         facTbl3[9,1]<-Ptd_RegionModels_asl_d[1]
         facTbl3[9,2]<-Ptd_RegionModels_asl_d[2]
         facTbl3[9,3]<-Ptd_RegionModels_asl_d[3]
	 facTbl3[9,4]<-Ptd_RegionModels_asl_d[4]
         facTbl3[9,5]<-Ptd_RegionModels_asl_d[5]
         facTbl3[9,6]<-Ptd_RegionModels_asl_d[6]
	 facTbl3[9,7]<-Ptd_RegionModels_asl_d[7]
         facTbl3[9,8]<-Ptd_RegionModels_asl_d[8]

         facTbl3[10,1]<-Sep_RegionModels_asl_d[1]
         facTbl3[10,2]<-Sep_RegionModels_asl_d[2]
         facTbl3[10,3]<-Sep_RegionModels_asl_d[3]
	 facTbl3[10,4]<-Sep_RegionModels_asl_d[4]
         facTbl3[10,5]<-Sep_RegionModels_asl_d[5]
         facTbl3[10,6]<-Sep_RegionModels_asl_d[6]
	 facTbl3[10,7]<-Sep_RegionModels_asl_d[7]
         facTbl3[10,8]<-Sep_RegionModels_asl_d[8]

         facTbl3[11,1]<-Soc_RegionModels_asl_d[1]
         facTbl3[11,2]<-Soc_RegionModels_asl_d[2]
         facTbl3[11,3]<-Soc_RegionModels_asl_d[3]
	 facTbl3[11,4]<-Soc_RegionModels_asl_d[4]
         facTbl3[11,5]<-Soc_RegionModels_asl_d[5]
         facTbl3[11,6]<-Soc_RegionModels_asl_d[6]
	 facTbl3[11,7]<-Soc_RegionModels_asl_d[7]
         facTbl3[11,8]<-Soc_RegionModels_asl_d[8]

         facTbl3[12,1]<-Sph_RegionModels_asl_d[1]
         facTbl3[12,2]<-Sph_RegionModels_asl_d[2]
         facTbl3[12,3]<-Sph_RegionModels_asl_d[3]
	 facTbl3[12,4]<-Sph_RegionModels_asl_d[4]
         facTbl3[12,5]<-Sph_RegionModels_asl_d[5]
         facTbl3[12,6]<-Sph_RegionModels_asl_d[6]
	 facTbl3[12,7]<-Sph_RegionModels_asl_d[7]
         facTbl3[12,8]<-Sph_RegionModels_asl_d[8]
...
facTbl3EFLong<-melt(facTbl3,id.vars="group",variable.name="region",value.name="EffectSize")
...
Fig4<-ggplot(facTbl3EFLong, aes(x=group, y=EffectSize,fill=region)) +
     ylab("Effect Size") + xlab("") + ggtitle("Regional CBF: Screening Diagnosis vs. Typically Developing") +
     geom_bar(stat="identity",position=position_dodge()) +
     scale_fill_manual(values=c("#325194","#B3141C","#F58311","#F6F612","#8E0EA2","#0EA240","#18A2F2","#FF66FF"), breaks=c("OverallPsy_RightPostcentralGyrus","OverallPsy_MidbrainSubgenACC","OverallPsy_LeftParahippocampalMidbrain",
     "OverallPsy_LeftRostralACC","OverallPsy_RightDorsalACC","Psychosis_LeftFrontalOperculumInsula","Fear_RightOccipFusiform","Fear_LeftSubgenACC"),
     labels=c("OverallPsy Right Postcentral Gyrus","OverallPsy Midbrain","OverallPsy Left Parahippocampal Gyrus/Midbrain","OverallPsy Left Rostral ACC","OverallPsy Right Dorsal ACC","Psychosis Left Frontal Operculum Insula",
     "Fear Right Occipital/Fusiform","Fear Left Subgenual ACC")) +
     ...
     theme(plot.title = element_text(size = rel(1.5), vjust = 0, hjust = .5)) + theme(axis.text.x = element_text(size = rel(1.25), colour="black")) +
     theme(axis.text.y = element_text(size = rel(1.25), colour="black")) + guides(fill=guide_legend(title=NULL)) +
     scale_x_discrete(breaks=c("Add","Agr","Con","Gad","Mdd","Ocd","Odd","Ps","Ptd","Sep","Soc","Sph"),
     labels=c("ADHD","Agoraphobia","Conduct","GAD","MDD","OCD","ODD","Psychosis","PTSD","Separation Anx","Social Anx","Specific Phobia")) +
...
ggsave(file="/data/joy/BBL/projects/pncAslAcrossDisorder/TablesFigures/FigureX_RegionByDiag.png", width = 15, height = 5, units = "in", dpi = 300)
