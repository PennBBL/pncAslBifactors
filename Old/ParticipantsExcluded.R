...
#medical rating 3/4 (GOAssess Medial History and CHOP EMR were used to define one summary rating for overall medical problems) (n=81)
data.ordered$ACROSS.INCLUDE.medical <- 1
data.ordered$ACROSS.INCLUDE.medical[data.ordered$medicalratingExclude==1] <- 0
medical.include<-sum(data.ordered$ACROSS.INCLUDE.medical)
medical.exclude<-1601-medical.include

#major incidental findings that distort anatomy (n=20)
data.ordered$ACROSS.INCLUDE.incidental <- 1
data.ordered$ACROSS.INCLUDE.incidental[data.ordered$incidentalFindingExclude==1] <- 0
incidental.include<-sum(data.ordered$ACROSS.INCLUDE.incidental)
incidental.exclude<-1601-incidental.include

#psychoactive medical medications (n=64)
data.ordered$ACROSS.INCLUDE.medicalMed <- 1
data.ordered$ACROSS.INCLUDE.medicalMed[data.ordered$psychoactiveMedMedicalv2==1] <- 0
medicalMed.include<-sum(data.ordered$ACROSS.INCLUDE.medicalMed)
medicalMed.exclude<-1601-medicalMed.include

#Count the total number excluded for healthExcludev2=1 (1=Excludes those with medical rating 3/4, major incidental findings that distort anatomy, psychoactive medical medications) (n=154, but medical.exclude + incidental.exclude + medicalMed.exclude = 165, so 11 people were excluded on the basis of two or more of these criteria)
data.ordered$ACROSS.INCLUDE.health <- 1
data.ordered$ACROSS.INCLUDE.health[data.ordered$healthExcludev2==1] <- 0
health.include<-sum(data.ordered$ACROSS.INCLUDE.health)
health.exclude<-1601-health.include

#Subset the data to just the 1447 that pass healthExcludev2
data.subset <-data.ordered[which(data.ordered$ACROSS.INCLUDE.health == 1), ]

#Count the number excluded for the proband sample (excludes those younger than 11 years because only collateral reports of psychiatric symptoms were available (Wolf et al., 2015 JAMA) (n=286)
data.subset$ACROSS.INCLUDE.11 <- 0
data.subset$ACROSS.INCLUDE.11[data.subset$age >= 11] <- 1
proband.include<-sum(data.subset$ACROSS.INCLUDE.11)
proband.exclude<-1447-proband.include

#Subset the data to just the 1161 proband sample (11 and up and that passed health exclusion)
data.subset2 <-data.subset[which(data.subset$ACROSS.INCLUDE.11 == 1), ]
