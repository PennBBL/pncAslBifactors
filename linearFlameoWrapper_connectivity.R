...
#Subject-level data
rds.name<-"/data/joy/BBL/projects/pncAslAcrossDisorder/subjectData/n833_JLF_connVox11andUp_subjData_conPaths.rds"

#Imaging modality
modality<-"REHO"

#Specify all variables that will be in the model for both naming and model inclusion (but no interaction terms).
variables<-c("age","sex","restRelMeanRMSMotion","Bifactor_Mood","Bifactor_Psychosis","Bifactor_Externalizing","Bifactor_Fear","Bifactor_Overall_Psychopathology")

#Define your model
lin.mod<-"~ age + sex + I(scale(age, scale=FALSE, center=TRUE)^2) + age*sex + I(scale(age, scale=FALSE, center=TRUE)^2)*sex + restRelMeanRMSMotion + Bifactor_Mood + Bifactor_Psychosis + Bifactor_Externalizing + Bifactor_Fear + Bifactor_Overall_Psychopathology"
...
if (modality=="REHO"){
  image.exclude<-"restExcludeVoxelwise"  #imaging exclusion criteria
  img<-"rACCPath"  #this is the column in the RDS where have list of paths to images
  smoothing<-0 #smooths the merged TS before running model- in sigma (rest data are already smoothed)
  mask<-"/data/joy/BBL/projects/pncAslAcrossDisorder/images/n1601_RestCoverageMask_GM.nii.gz"                 #reho
  outroot<-"/data/joy/BBL/projects/pncAslAcrossDisorder/results" }
